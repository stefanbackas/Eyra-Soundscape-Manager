...
class AudioPlayer(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Ljudspelar Program")
    ...
    def play_audio(self, filename):
        ...
        # Start the process from the paused position if paused, otherwise from 0
        start_position = file_info['position'] if file_info and file_info['paused'] else 0
        process = subprocess.Popen(['ffplay', '-nodisp', '-autoexit', '-loglevel', 'quiet', '-ss', str(start_position), file_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
        self.file_processes[filename] = {
            'process': process,
            'duration': duration,
            'position': start_position,  # Keep track of where the playback starts
            'paused': False  # Set paused state to False
        }
        print(f"Spelar upp fil: {filename} från {start_position} sekunder")

        # Add controls for this file to the right column
    ...
    def seek_audio(self, filename, position, from_slider):
        if filename in self.file_processes:
            # If slider is moved, only seek without playing
            if from_slider and self.file_processes[filename]['paused']:
                ...
                # Stop the current process and seek to the new position
                self.pause_audio(filename)
                file_path = self.file_paths[filename]
                ...
                self.file_processes[filename] = {
                    ...
                    'position': position,
                    'paused': False  # Mark as playing, not paused
                }

...
# Funktion för att skriva schemat till en textfil i önskat format
def write_schedule_to_file(schedule, filename):
    with open(filename, "w") as file:
        for pool, times in schedule.items():
            # Ta bort mellanslaget före varje kommatecken
            pool_times = ','.join(f'"* * {t["start_time"]}>{t["end_time"]} * * * *"' for t in times)
            file.write(f"Pool {pool + 1}: {pool_times}\n")
...
output_file = "C:/Users/stefanbackas/Documents/000_EYRA/Kod/schema.txt"
...
# Skriv ut schemat till fil
write_schedule_to_file(schedule, output_file)

print(f"\nSchemat har skrivits till {output_file}")